...
export default function Licznik({ wartosc, sufiks = "", etykieta }: Props) {
  ...
  return (
    <div
      ref={karta}
      className="panel-hud group relative px-5 py-6 text-center transition-colors duration-300 hover:border-huk-red/70"
      style={{ clipPath: "polygon(12px 0, 100% 0, 100% calc(100% - 12px), calc(100% - 12px) 100%, 0 100%, 0 12px)" }}
    >
      <div className="font-display text-[clamp(2rem,5vw,3.4rem)] leading-none text-huk-white tabular-nums">
        <span ref={liczba}>0</span>
        <span className="text-huk-red">{sufiks}</span>
      </div>
      <div className="podtytul mt-3 text-white/65 transition-colors duration-300 group-hover:text-white/70">
        {etykieta}
      </div>
    </div>
  );
